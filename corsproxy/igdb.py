...
def _get_token():
    ...
    with _token_lock:
        if _token and time.monotonic() < _token_expires:
            return _token
        r = _session.post(_TWITCH, params={
...
# ── Canonical game selection (resolve parent/version inline) ──────────────────
def _pick_canonical(g):
    """Return version_parent > parent_game (unless remake/remaster) > self."""
    cat = g.get('category') if g.get('category') is not None else (g.get('game_type') or {}).get('id', 0)
    vp = g.get('version_parent')
    if isinstance(vp, dict) and vp.get('id'):
        return vp
    if cat not in (8, 9):
        pg = g.get('parent_game')
...
def fetch_by_name(name: str, pub_ts: int = None):
    ...
    if pub_ts:
        ...
        if in_window:
            results = in_window
    return _make_result(_rank(results, name)[0])
