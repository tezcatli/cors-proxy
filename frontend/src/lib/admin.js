...
// Client for the account-administration endpoints (`/auth/*`). Kept out
...
export async function fetchInvitations() {
  const r = await apiFetch('/auth/invitations')
...
export async function sendInvitation(email, isAdmin = false) {
  const r = await apiFetch('/auth/invite', {
    method:  'POST',
    headers: JSON_HEADERS,
...
export async function revokeInvitation(token) {
  await apiFetch(`/auth/invitations/${encodeURIComponent(token)}`, { method: 'DELETE' })
}
