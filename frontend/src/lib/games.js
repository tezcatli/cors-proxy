...
export async function fetchResolutionStats() {
  const r = await apiFetch('/games/resolution-stats')
  return r.json()
}

export async function searchIgdb(query) {
  const r = await apiFetch(`/games/igdb-search?q=${encodeURIComponent(query)}`)
...
export async function setCorrection(target) {
  const r = await apiFetch('/games/corrections', { method: 'PUT', ...correctionBody(target) })
  return r.json()
}

export async function deleteCorrection(target) {
  const r = await apiFetch('/games/corrections', { method: 'DELETE', ...correctionBody(target) })
  return r.json()
}

export async function refreshPodcastIgdb(podcastId) {
  const r = await apiFetch(`/games/podcasts/${encodeURIComponent(podcastId)}/igdb-refresh`,
                           { method: 'POST' })
  return r.json()
}
...
async function fetchStreamToken() {
  const r = await apiFetch('/auth/stream-token')
  return (await r.json()).token
}

export async function openResolutionStream() {
  const token = await fetchStreamToken()
  return new EventSource(`/games/resolution-stream?token=${encodeURIComponent(token)}`)
}
