...
- **`src/lib/admin.js`** — API client for the `/silence/auth/*` account-admin endpoints. Separate from `lib/auth.js` (which every session loads) so it ships with the admin chunk; unlike `apiFetch`'s other callers it must set `Content-Type: application/json` explicitly, or Flask's `get_json()` sees nothing.
